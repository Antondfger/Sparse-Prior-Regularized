...
def session_split(
    ...
    Args:
        ...
        unpair (boolean): Pair with separation mode. Defaults to False.
    ...
    if unpair:
        train, test = make_unpair(train, test, user_id, timestamp)
...
def make_unpair(train, test, user_id="user_id", timestamp="timestamp"):
    """Randomly swaps neighboring pairs with the same timestamp."""

    train[timestamp] += np.random.rand(len(train))
    test[timestamp] += np.random.rand(len(test))
    train.sort_values([user_id, timestamp], inplace=True)
    test.sort_values([user_id, timestamp], inplace=True)
...
def make_validation(
    ...
    validation_users = np.random.choice(
        train[user_id].unique(), size=validation_size, replace=False
    )
