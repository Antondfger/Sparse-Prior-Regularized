...
from nn.datasets import CausalLMDataset, CausalLMPredictionDataset, PaddingCollateFn
...
def main(config):
    ...
    evaluate(
        ...
        task,
        config,
        prefix=f"test_fs_{seqrec_module.filter_seen}",
    )
...
def training(model, train_loader, eval_loader, train, max_item_id, config):
    ...
    if config.dataset_params.num_negatives is not None:
        seqrec_module = SeqRecWithSampling(
            model,
    ...
    trainer = pl.Trainer(
        callbacks=callbacks, enable_checkpointing=True, **config["trainer_params"]
    )
