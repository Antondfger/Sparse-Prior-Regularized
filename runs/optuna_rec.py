...
from runs.dl import create_dataloaders, create_model, training, predict

@hydra.main(config_path="conf", config_name="GRU4Rec_optuna")
def main(config):
    ...
    train_loader, eval_loader = create_dataloaders(train, validation_inputs,
                                                   config)
    ...
    recs_validation = predict(trainer, seqrec_module, validation_inputs,
                              config)

    val_metrics = evaluate(recs_validation, validation_gt, train, task, config, prefix='val')
